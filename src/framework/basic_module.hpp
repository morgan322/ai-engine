...
public:
  ...
  void
  SetProcessDoneCallback(std::function<int(std::shared_ptr<Frame>)> callback) {
    callback_ = callback;
  }

  virtual int Transmit(std::shared_ptr<Frame> frame) final { // NOLINT
    if (callback_)
...
public:
  virtual int Open() = 0;
  virtual int Process(std::shared_ptr<Frame> frame) = 0;
  virtual int Close() = 0;

private:
  std::string module_name_ = "";
  int parallelism_ = 1;
  std::function<int(std::shared_ptr<Frame>)> callback_;
