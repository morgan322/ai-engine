...
static int GetDeviceInfo(int device_id, DeviceInfo *info) {
  ...
  //   LOG(ERROR) << "[EasyDK] GetDeviceInfo(): device id is invalid, device_id: "
  //              << device_id << ", total count: " << count;
  //   return -1;
  // }
  ...
  DeviceInfo dev_info;
  // AIDeviceProp_t prop;
  // CNRT_SAFECALL(cnrtGetDeviceProperties(&prop, device_id),
  //               "GetDeviceInfo(): failed", -1);

  // VLOG(2) << "[EasyDK] GetDeviceInfo(): device id: " << device_id
